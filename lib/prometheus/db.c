...
#define PROM_DB_SQLITE_MAX_RETRY_COUNT		20
#define PROM_DB_SQLITE_MAX_RETRY_DELAY_MS	100
...
static int db_busy(void *user_data, int busy_count) {
  ...
  if (busy_count <= PROM_DB_SQLITE_MAX_RETRY_COUNT) {
    retry = TRUE;
  }
...
static int get_schema_version(pool *p, struct prom_dbh *dbh,
  ...
  if (results->nelts != 1) {
    pr_log_debug(DEBUG3, MOD_PROMETHEUS_VERSION
...
static int set_schema_version(pool *p, struct prom_dbh *dbh,
    const char *schema_name, unsigned int schema_version) {
  int res, xerrno = 0;
  const char *stmt, *errstr = NULL;
  array_header *results;
...
static void check_db_integrity(pool *p, struct prom_dbh *dbh, int flags) {
  int res;
  const char *stmt, *errstr = NULL;
